...
class win_Login(QMainWindow):
    ...
    def onSignIn(self):
        ...
        if username not in USER_PWD.keys():
            replay = QMessageBox.warning(self,"False!", "Error", QMessageBox.Yes)
        else:
            # 若登陆成功，则跳转主界面
            if USER_PWD.get(username) == password:
                ...
                replay = QMessageBox.warning(self, "!", "Error", QMessageBox.Yes)
...
class win_Register(QDialog):
    ...
    def new_account(self):
        ...
        # 判断用户名是否为空
        if new_username == "":
            replay = QMessageBox.warning(self, "!", "Cannot be Empty", QMessageBox.Yes)
        else:
            # 判断账号是否存在
            if new_username in USER_PWD.keys():
                replay = QMessageBox.warning(self, "!", "Already Exist", QMessageBox.Yes)
            else:
                # 判断密码是否为空
                if new_password == "":
                    replay = QMessageBox.warning(self, "!", "Cannot be Empty", QMessageBox.Yes)
