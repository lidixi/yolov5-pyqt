...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    def init_slots(self):
        ...
        # self.radioButton.setText.clicked.connect(self.monitor)

    # 打开权重文件
    def open_model(self):
        self.openfile_name_model, _ = QFileDialog.getOpenFileName(self.ui.pushButton_weights, 'Select Model',
                                                             'weights/')
        if not self.openfile_name_model:
            QtWidgets.QMessageBox.warning(self, u"Warning", u"open failed", buttons=QtWidgets.QMessageBox.Ok,
                                          defaultButton=QtWidgets.QMessageBox.Ok)
        else:
            print('Model Location ：' + str(self.openfile_name_model))
    ...
    def model_init(self):
        ...
        # 设置提示框
        QtWidgets.QMessageBox.information(self, u"Notice", u"Load Successful", buttons=QtWidgets.QMessageBox.Ok,
                                      defaultButton=QtWidgets.QMessageBox.Ok)
        ...
        with torch.no_grad():
            ...
            for i, det in enumerate(pred):
                if det is not None and len(det):
                    ...
                    #     for i, txt in enumerate(Result.split('\n')):
                    #         y = 100 + i * 40
                    #         cv2.putText(img, txt, (50, y), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
    ...
    def button_image_open(self):
        ...
        try:
            ...
            print('Cannot Found'+ str(reason))
        else:
            # 判断图片是否为空
            if not img_name:
                QtWidgets.QMessageBox.warning(self, u"Warning", u"Open Image Failed", buttons=QtWidgets.QMessageBox.Ok,
                                              defaultButton=QtWidgets.QMessageBox.Ok)
            else:
    ...
    def button_video_open(self):
        ...
        if not flag:
            QtWidgets.QMessageBox.warning(self, u"Warning", u"Open Video Failed", buttons=QtWidgets.QMessageBox.Ok,defaultButton=QtWidgets.QMessageBox.Ok)
        else:
            # 写入视频
    ...
    def button_camera_open(self):
        ...
        if not bool_open:
            QtWidgets.QMessageBox.warning(self, u"Warning", u"Open Camera Failed", buttons=QtWidgets.QMessageBox.Ok,
                                          defaultButton=QtWidgets.QMessageBox.Ok)
        else:
            fps, w, h, save_path = self.set_video_name_and_path()
            fps = 10 # 控制摄像头检测下的fps，Note：保存的视频
            self.vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
    ...
    def button_video_stop(self):
        ...
        if self.timer_video.isActive() == True and self.num_stop%2 == 1:
            self.ui.pushButton_stop.setText(u'Continue Detect') # 当前状态为暂停状态
            self.num_stop = self.num_stop + 1 # 调整标记信号为偶数
            self.timer_video.blockSignals(True)
        # 继续检测
        else:
            self.num_stop = self.num_stop + 1
            self.ui.pushButton_stop.setText(u'Stop Detect')
